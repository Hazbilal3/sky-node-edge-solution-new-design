...
const cfg: SolutionConfig = {
  ...
  heroSub: 'Receiver sites extend two-way radio coverage into areas your transmitters can\'t reach on their own — buildings, underground, and terrain gaps. Skynode nodes are evaluated for the receive path quality and backhaul that voted receiver deployments depend on.',
  heroCard: {
    title: 'Receiver Site — What to Expect',
    items: [
      { label: 'Elevation', value: 'TBC per site evaluation' },
      { label: 'Receive path quality', value: 'Evaluated per system and coverage need' },
      { label: 'Voter connectivity backhaul', value: 'IP backhaul — fiber / P2P / Metro Fabric' },
      { label: 'Power requirement', value: 'Lower than transmitter — TBC per site' },
  ...
  trust: {
    ...
    body: 'Voted receiver systems improve coverage where subscribers transmit from marginal locations — parking garages, lower floors, terrain shadows. Getting receiver sites right requires attention to elevation, RF environment quality, and the IP backhaul that connects them to the voter controller.',
    features: [
      ...
      { title: 'Strategic placement for coverage fills', body: 'Receiver sites are most valuable when placed strategically — covering subscriber transmit shadows not addressed by your transmitter sites. Skynode can help identify node placements that complement your existing transmitter infrastructure.' },
      { title: 'Simplified multi-site coordination', body: 'When transmitter and receiver sites are both on Skynode nodes in the same market, Metro Fabric interconnection provides direct low-latency paths between them — simplifying voter backhaul architecture.' },
    ],
  },
  specs: [
    ...
    { label: 'Power requirement', value: 'Low — receive only', note: 'No transmit amplification' },
  ...
  useCases: [
    ...
    { name: 'Coverage fill for subscriber weak zones', body: 'Areas where subscriber handhelds transmit at marginal power — lower building floors, parking structures, and terrain shadows — are ideal targets for additional receive sites.', tags: ['Coverage Fill', 'Subscriber', 'Marginal'] },
